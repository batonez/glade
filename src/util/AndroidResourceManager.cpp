...
std::shared_ptr<ShaderProgram> Glade::ResourceManager::loadShaderProgram(const Path &vertex_shader_filename, const Path &fragment_shader_filename)
{
  ...
  shaderProgram.get()->vertexShaderSource.assign(gVertexShader, gVertexShader + strlen(gVertexShader));
  shaderProgram.get()->fragmentShaderSource.assign(gFragmentShader, gFragmentShader + strlen(gFragmentShader));
